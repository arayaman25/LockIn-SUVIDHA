...
import { ScoredPartner, NearbyPartnersRequest } from '@/src/types/scheme-matching';
...
const PartnerMap = dynamic(() => import('@/components/PartnerMap'), {
  ...
  loading: () => (
    <div className="w-full h-full min-h-[460px] lg:min-h-[620px] rounded-2xl border border-outline-variant/60 bg-surface-container flex flex-col items-center justify-center text-center p-6 text-on-surface-variant">
      ...
      <p className="text-xs text-on-surface-variant mt-1">Locating verified bank branches and citizen desks</p>
    </div>
  ),
});
...
const SCHEME_OPTIONS = [
  { id: 'pm-svanidhi', label: 'PM SVANidhi' },
  { id: 'mudra-kishore', label: 'PM MUDRA (Kishore)' },
  { id: 'standup', label: 'Stand-Up India' },
  { id: 'csis', label: 'Education Subsidy (CSIS)' },
  { id: 'pm-vishwakarma', label: 'PM Vishwakarma' },
];
...
export default function PartnerLocator() {
  ...
  const locationRequest: NearbyPartnersRequest | null = userCoords
    ? { citizenLat: userCoords.lat, citizenLng: userCoords.lng, limit: 20 }
    : null;
  ...
  const activeMapPartner = mapPartners.find((p) => p.id === selectedPartnerId) ?? mapPartners[0] ?? null;

  const handleUseMyLocation = useCallback(() => {
    if (typeof window === 'undefined' || !navigator.geolocation) {
      setGeoError('Geolocation is not supported by your browser. Please search for a city or area instead.');
      return;
    }
    setIsLocating(true);
    setGeoError(null);
    navigator.geolocation.getCurrentPosition(
      ...
      (err) => {
        ...
        if (err.code === err.PERMISSION_DENIED) {
          ...
          setGeoError('Location access failed. Please search for your area instead.');
        }
      },
  ...
  const handleSelectFromMap = useCallback((partner: ReturnType<typeof toMapPartner>) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-xs text-on-surface-variant mt-0.5">
            Locate nearest Public Sector Bank branches, Gramin banks, and CSC digital assistance nodes.
          </p>
        </div>
      ...
      <p className="text-[11px] text-on-surface-variant bg-surface-container px-3 py-2 rounded-xl border border-outline-variant/40 w-fit">
        🔒 Your location is used only to find nearby authorized partner agencies and is not stored.
      </p>

      {/* Geolocation error */}
      {geoError && (
        <div role="alert" className="flex items-start gap-2 p-3 rounded-xl bg-error-container/20 border border-error/30 text-xs text-on-error-container">
          <Icon name="location_off" className="w-4 h-4 shrink-0 mt-0.5 text-error" />
          <span>{geoError}</span>
        </div>
      )}
        ...
        <div className="flex flex-wrap gap-2">
          {SCHEME_OPTIONS.map((scheme) => (
            <button
              key={scheme.id}
              type="button"
              onClick={() => { setSelectedSchemeId(scheme.id); setSelectedPartnerId(null); }}
      ...
      {!hasLocation && (
        <div className="p-8 text-center bg-surface-container-lowest rounded-2xl border border-outline-variant/50 shadow-xs">
          <Icon name="location_searching" className="w-10 h-10 text-outline mb-3 mx-auto" />
          ...
          <p className="text-xs text-on-surface-variant mt-1.5 leading-relaxed max-w-sm mx-auto">
            Click <strong>Use My Location</strong> to detect your position and find nearby partners.
          </p>
        </div>
      )}
      ...
      {hasLocation && !hasScheme && (
        <div className="p-6 text-center bg-surface-container-lowest rounded-2xl border border-outline-variant/50 shadow-xs">
          ...
          <p className="font-bold text-primary text-sm">Select a scheme above</p>
          <p className="text-xs text-on-surface-variant mt-1">
            Select a scheme to find authorized nearby channel partners for that programme.
          </p>
        </div>
      )}
          ...
          {(['both', 'map', 'list'] as const).map((tab) => (
            <button
              ...
              className={`px-2.5 py-1 rounded-lg transition-all ${
                mobileTab === tab ? 'bg-primary text-white shadow-xs' : 'text-on-surface'
              }`}
            >
              {tab === 'both' ? 'Split' : tab === 'map' ? 'Map View' : 'List View'}
            </button>
          ))}
        </div>
      )}
      ...
      {readyToQuery && (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
          ...
          >
            {isLoadingPartners && (
              <div className="p-8 text-center">
                ...
                <p className="text-xs text-on-surface-variant">Searching for nearby authorized partners…</p>
              </div>
            )}

            {isPartnersError && !isLoadingPartners && (
              <div className="p-6 rounded-2xl bg-error-container/20 border border-error/30 text-center">
                <Icon name="error_outline" className="w-8 h-8 text-error mx-auto mb-2" />
                <p className="text-xs font-bold text-on-error-container">
                  {partnersError?.message || 'Could not load nearby partners. Please try again.'}
                </p>
              </div>
            )}
            ...
            {!isLoadingPartners && !isPartnersError && mapPartners.length === 0 && (
              <div className="p-8 text-center bg-surface-container-lowest rounded-2xl border border-outline-variant/50 shadow-xs">
                ...
                <p className="font-bold text-primary text-sm">No authorized channel partners found nearby.</p>
            ...
            {!isLoadingPartners && mapPartners.map((partner) => {
                ...
                >
                  <div className="flex justify-between items-start gap-2">
                    <div className="flex-1 min-w-0">
                      ...
                      <h4 className="font-bold text-primary text-sm leading-snug">{partner.name}</h4>
                      {partner.address && (
          ...
          {/* Map */}
          <div className={`lg:col-span-7 h-[480px] lg:h-[620px] sticky top-4 ${
