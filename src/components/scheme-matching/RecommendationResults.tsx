...
/**
 * Matches are presented on the recommendations page, so the wizard's final
 * step only covers the moments before that hand-off: searching (which also
 * spans the redirect), finding nothing, or failing to reach the engine.
 */
export type RecommendationSubmissionStatus =
  | { status: 'searching' }
  | { status: 'no-match'; message?: string }
  | { status: 'error'; message: string };

interface RecommendationResultsProps {
  submission: RecommendationSubmissionStatus;
  onRetry: () => void;
...
export default function RecommendationResults({
  ...
  // 1. Searching State
  if (submission.status === 'searching') {
    return (
    ...
    return (
      <div className="bg-surface rounded-2xl border border-red-200 p-8 sm:p-12 shadow-sm text-center">
        ...
        <p className="text-on-surface-variant max-w-md mx-auto text-sm leading-relaxed mb-6">
          {submission.message || 'The scheme recommendation engine could not be reached. Please check your connection or try again.'}
        </p>
